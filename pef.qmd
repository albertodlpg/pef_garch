...
La volatilidad es una de las variables centrales en los mercados financieros porque resume la incertidumbre asociada al comportamiento futuro de los precios y funciona como un insumo crítico para la gestión de riesgo, la valuación de derivados, la asignación de portafolios y la toma de decisiones de inversión. En contextos de estabilidad, una estimación razonable de la volatilidad permite dimensionar pérdidas potenciales, definir coberturas y valorar instrumentos financieros con mayor precisión. En periodos de estrés como crisis financieras, episodios de alta inflación, shocks geopolíticos o eventos extraordinarios como la pandemia de COVID-19, su relevancia aumenta de forma significativa porque pequeños errores de medición pueden amplificarse en pérdidas económicas relevantes, requerimientos de capital mal calibrados o decisiones estratégicas subóptimas.

El problema adquiere mayor importancia porque la volatilidad no es directamente observable. A diferencia del precio de un activo, que puede observarse en el mercado, la volatilidad debe estimarse a partir de información histórica, precios de derivados o modelos estadísticos. Esta característica introduce un reto metodológico donde distintos modelos pueden generar estimaciones distintas para el mismo activo incluso cuando utilizan la misma información de mercado. Por ello, la elección del modelo de volatilidad no es una decisión puramente técnica, sino una decisión con implicaciones económicas directas.

En el caso de la valuación de opciones, la precisión en la estimación de volatilidad es especialmente relevante. En el modelo Black-Scholes, la volatilidad es el único insumo no observable y tiene un efecto directo sobre el precio teórico del derivado. Una subestimación puede producir precios artificialmente bajos y generar primas insuficientes frente al riesgo asumido. Una sobreestimación puede encarecer las opciones y limitar operaciones potencialmente rentables. En ambos escenarios, el resultado es una asignación ineficiente de recursos y una pérdida de precisión en la medición del valor económico del instrumento.

La literatura financiera ha documentado que los retornos de los activos presentan hechos estilizados incompatibles con una volatilidad constante. Entre estos destacan la presencia de colas pesadas, leptocurtosis, clustering de volatilidad, persistencia, reversión a la media y respuestas asimétricas ante choques positivos y negativos. Estos patrones justifican el uso de modelos de volatilidad condicional, particularmente la familia ARCH/GARCH y sus extensiones, porque permiten que la varianza esperada dependa de información pasada y se ajuste dinámicamente a las condiciones del mercado.

Sin embargo, no existe un consenso absoluto sobre cuál especificación GARCH es superior en todos los contextos. Un modelo puede mostrar buen ajuste dentro de muestra y perder capacidad predictiva fuera de muestra. Otro puede pronosticar adecuadamente la varianza realizada y no generar los mejores precios de opciones. Esta distinción es fundamental porque el objetivo final de un modelo financiero no siempre es minimizar una métrica estadística, sino mejorar una decisión económica concreta. Bajo esta lógica, evaluar modelos de volatilidad únicamente por su ajuste estadístico puede ser insuficiente cuando el uso final es la valuación de derivados.

Este estudio se sitúa precisamente en esa intersección entre precisión estadística y utilidad económica. Al analizar el Dow Jones Industrial Average durante el periodo 2000-2025, el proyecto evalúa si distintos modelos de la familia GARCH no solo capturan la dinámica de la volatilidad del mercado, sino también si sus pronósticos aportan valor tangible al incorporarse en el *pricing* de opciones. De esta forma, el problema de investigación no se limita a identificar qué modelo predice mejor la volatilidad, sino a determinar si esa mejora se traduce en precios de opciones más cercanos a los observados en el mercado. 

## Preguntas de investigación

En respuesta a los vacíos identificados, este trabajo formula dos preguntas de investigación principales: 1. ¿Cuál modelo de la familia GARCH, entre ARCH, GARCH, EGARCH, GJR-GARCH y APARCH, ofrece el mejor desempeño predictivo para la volatilidad del DJIA durante el periodo 2000-2025, consierando tanto criterios de ajuste *in-sample* como métricas de precisión *out-of-sample* bajo diferentes esquemas de estimación? 2. ¿En qué medida las mejoras estadísticas, en el pronóstico de la volatilidad, generan valor económico real medido a través de la precisión en *pricing* de opciones financieras?
...
La principal contribución de este estudio consiste en integrar, dentro de un mismo marco empírico, la evaluación estadística de modelos de volatilidad y su aplicación económica en la valuación de opciones financieras. En lugar de limitar la comparación a criterios tradicionales de ajuste o a métricas de pronóstico de varianza, el trabajo conecta los modelos GARCH con una aplicación concreta de mercado que corresponde al cálculo de precios teóricos de opciones mediante Black-Scholes utilizando volatilidad pronosticada como insumo. Esto permite evaluar si las mejoras estadísticas realmente generan valor económico medible.

El estudio también contribuye al comparar de forma sistemática cinco especificaciones de la familia GARCH. Las especificaciones analizadas son ARCH(1), GARCH(1,1), EGARCH(1,1), GJR-GARCH(1,1,1) y APARCH(1,1,1). Esta selección permite contrastar modelos simétricos y asimétricos junto con especificaciones con distintos niveles de flexibilidad paramétrica. La inclusión de EGARCH, GJR-GARCH y APARCH permite analizar si capturar efectos de apalancamiento, asimetría y colas pesadas mejora la predicción de volatilidad y el desempeño en *pricing* frente a modelos más simples.

Otra contribución relevante es la comparación entre dos esquemas de estimación que corresponden a ventana fija y ventana móvil. Esta dimensión es importante porque la muestra 2000-2025 incluye episodios de cambio estructural y alta volatilidad como la crisis dot-com, la crisis financiera global, la pandemia de COVID-19 y el periodo posterior de ajuste. Al contrastar ambos esquemas, el estudio evalúa si resulta más conveniente preservar una base histórica amplia o actualizar los parámetros con mayor sensibilidad a la información reciente. Así, la elección de ventana deja de ser un detalle operativo y se convierte en una dimensión central del análisis.

El proyecto aporta además evidencia empírica específica para el Dow Jones Industrial Average, un índice de referencia con características distintas a otros benchmarks ampliamente estudiados. La base de retornos diarios permite construir pronósticos de varianza acumulada a 21 días, horizonte que posteriormente se alinea con la muestra de opciones sobre DJX. Esta consistencia temporal fortalece el vínculo entre la evaluación estadística de volatilidad y la aplicación económica en opciones con vencimiento comparable.

En términos de resultados, el estudio muestra que la selección del “mejor” modelo depende del objetivo de evaluación. APARCH presenta el mejor ajuste *in-sample* bajo AIC y BIC. EGARCH con ventana fija obtiene el mejor desempeño *out-of-sample* frente a varianza realizada bajo QLIKE. GJR-GARCH con ventana móvil genera el menor error de *pricing* en las categorías de *moneyness* analizadas. Esta evidencia contribuye al debate metodológico porque demuestra que precisión estadística y utilidad económica no son equivalentes y que el modelo óptimo debe seleccionarse en función del uso final.

Finalmente, el estudio documenta que incorporar volatilidad condicional dinámica mejora el *pricing* frente al *benchmark* tradicional de volatilidad histórica de 252 días. La reducción del error es particularmente relevante en opciones *at-the-money* y *near-the-money*, donde la sensibilidad del precio a la volatilidad es mayor. Con ello, el trabajo aporta evidencia de que los modelos GARCH no solo capturan regularidades estadísticas de los retornos, sino que también pueden mejorar decisiones financieras concretas cuando se aplican de forma alineada con el horizonte y objetivo económico del instrumento.

## Estructura del documento

El documento se organiza en siete secciones. La primera sección presenta la introducción del estudio, donde se expone el contexto y la relevancia del problema, se plantean las preguntas de investigación, se definen los objetivos generales y específicos, se establece la contribución del trabajo y se describe la estructura general del documento.

La segunda sección desarrolla la revisión literaria. En ella se abordan los principales hechos estilizados de la volatilidad financiera. Entre ellos se incluyen colas pesadas, clustering, persistencia, reversión a la media y asimetría. Posteriormente se revisan los modelos simétricos ARCH y GARCH junto con las extensiones asimétricas EGARCH, GJR-GARCH y APARCH. También se discuten los criterios de evaluación *in-sample* y *out-of-sample*, los esquemas de estimación mediante ventana fija y ventana móvil, la relación entre precisión estadística y valor económico y la brecha empírica que justifica la aplicación al DJIA.

La tercera sección presenta la metodología. Primero se describe la construcción de la base de datos de retornos diarios del Dow Jones Industrial Average y el cálculo de la varianza realizada a 21 días. Después se justifica estadísticamente el uso de modelos GARCH mediante pruebas de estacionariedad, autocorrelación y dependencia en segundos momentos. Posteriormente se especifican los modelos de volatilidad utilizados, los esquemas de estimación, la generación de pronósticos de varianza acumulada y la métrica QLIKE para evaluación estadística. Finalmente, se detalla la aplicación financiera mediante Black-Scholes, la conversión de pronósticos GARCH a volatilidad anualizada, el *benchmark* de volatilidad histórica, la base de opciones DJX y las métricas de error en *pricing*.

La cuarta sección reporta los resultados empíricos. En primer lugar, se presenta la evidencia descriptiva inicial de los retornos del DJIA incluyendo estadísticos, distribución empírica, pruebas de diagnóstico y evidencia visual de clustering de volatilidad. Después se muestran los resultados *in-sample* bajo criterios AIC y BIC seguidos por la evaluación *out-of-sample* de pronósticos de varianza mediante QLIKE. La sección cierra con la aplicación económica al *pricing* de opciones, donde se comparan los errores de valuación por categoría de *moneyness* y se evalúa el desempeño de los modelos frente al *benchmark* de volatilidad histórica.

La quinta sección presenta las conclusiones del estudio. En esta parte se sintetizan los principales hallazgos, se discute la diferencia entre desempeño estadístico y desempeño económico, se interpreta la superioridad de GJR-GARCH con ventana móvil en *pricing* de opciones y se señalan las limitaciones del trabajo. También se proponen líneas futuras de investigación que incluyen la extensión del análisis a otros activos, otros mercados, distintos vencimientos, métricas de cobertura y modelos de *pricing* que incorporen directamente dinámicas GARCH bajo una medida neutral al riesgo.

La sexta sección contiene la bibliografía utilizada, incluyendo las referencias fundamentales sobre modelos ARCH/GARCH, hechos estilizados de volatilidad, evaluación de pronósticos, selección de ventanas, modelos de *pricing* de opciones y aplicaciones empíricas relacionadas. Finalmente, la séptima sección incluye los apéndices, donde se presentan gráficos complementarios de errores en los pronósticos de varianza realizada y diagnósticos de dispersión relativa entre varianza pronosticada y varianza realizada para cada modelo y esquema de estimación.
...
Para el análisis se selecciona el subconjunto de observaciones con 21 días al vencimiento. Esto permite que las observaciones estén alineados con el horizonte de pronósitco de volatilidad utilizado en el estudio.

El total de observaciones en la base de datos utilizada es de 10,152 observaciones, distribuidos entre 5,076 calls y 5,076 puts con vencimientos entre el 16 de febrero de 2018 y el 19 de septiembre de 2025.
...
![Retornos Logarítmicos Diarios del DJIA](figures/fig-returns.png){#fig-returns}
...
: Estadísticos descriptivos de los retornos diarios del DJIA

La evidencia descriptiva se refuerza visualmente en la @fig-dist al comparar la distribución empírica de los retornos con una distribución normal y una Student-t ajustada.
...
: Pruebas de estacionariedad y autocorrelación

La evidencia de dependencia en la varianza también se observa directamente en la serie de varianza diaria realizada, como se muestra en la @fig-variance.
...
| \# Modelo | Modelo de media | Proceso de volatilidad | Distribución | AIC | BIC |
|-----------:|------------|------------|------------|-----------:|-----------:|
...
## Pronóstico *out-of-sample* de Varianza

La evaluación fuera de muestra cubre el periodo del 2 de enero de 2018 al 1 de diciembre de 2025, con 1990 observaciones de pronóstico de varianza acumulada a 21 días, lo cual es consistente con la idea de que la volatilidad relevante para una opción debe reflejar el riesgo durante la vida del contrato. La métrica utilizada para evaluar el desempeño predictivo es QLIKE, donde valores más bajos indican mejor desempeño predictivo. 
...
: QLIKE out-of-sample contra Varianza Realizada
...
A modo ilustrativo, la @fig-comparison muestra la evolución temporal de los pronósticos de varianza acumulada a 21 días para los modelos EGARCH(1,1) bajo ventana fija y GARCH(1,1) bajo ventana fija y rolling, comparados contra la varianza realizada.

![Comparación de Pronósticos de Varianza contra Varianza Realizada](figures/fig-comparison.png){#fig-comparison}

Otra forma de interpretar los errores en pronóstico de varianza es como la diferencia entre la varianza realizada y la varianza pronosticada. La @sec-apendice-a muestra el conjunto de gráficas que ilustran estos errores para cada modelo y esquema de estimación. En general, los modelos con menor QLIKE presentan errores más concentrados alrededor de cero, mientras que ARCH, especialmente bajo rolling window, muestra errores más amplios y con mayor dispersión.

Los diagnósticos visuales modelo por modelo se reportan en la @sec-apendice-b. En conjunto, estos gráficos confirman que las especificaciones con menor QLIKE presentan menor dispersión relativa frente a la volatilidad realizada, mientras que ARCH, especialmente bajo rolling window, muestra errores más amplios y menor capacidad para capturar episodios de volatilidad elevada.
...
La muestra final contiene 10,152 observaciones de opciones que se clasificaron por *moneyness* para evaluar si el desempeño de los modelos cambia según la posición relativa del strike y el subyacente. La clasificación utiliza la razón de *moneyness* definida como $\mathrm{MNY}$ de forma que valores cercanos a uno representen opciones *at-the-money* tanto para *calls* como para *puts*.

| Categoría          | Rango de moneyness                                            | Observaciones |
|--------------------|---------------------------------------------------------------|--------------:|
| ATM                | $0.97 \le \mathrm{MNY} \le 1.03$                             |         1,144 |
| Near-the-money     | $0.94 < \mathrm{MNY} < 0.97$ o $1.03 < \mathrm{MNY} < 1.06$   |         1,077 |
| Moderately OTM/ITM | $0.88 < \mathrm{MNY} \le 0.94$ o $1.06 \le \mathrm{MNY} < 1.12$ |         1,693 |
| Deep OTM/ITM       | $\mathrm{MNY} < 0.88$ o $\mathrm{MNY} \ge 1.12$              |         6,238 |

: Distribución de Contratos por Categoría de Moneyness
...
| Categoría          | Ranking | Modelo                        |   RMSE |    MAE |
|--------------------|--------:|-------------------------------|-------:|-------:|
| ATM                |       1 | GJR-GARCH(1,1) Rolling-Window | 1.0205 | 0.7428 |
| ATM                |       2 | APARCH(1,1) Fixed-Window      | 1.0375 | 0.7702 |
| ATM                |       3 | APARCH(1,1) Rolling-Window    | 1.0919 | 0.7952 |
| Near-the-money     |       1 | GJR-GARCH(1,1) Rolling-Window | 0.8840 | 0.6166 |
| Near-the-money     |       2 | APARCH(1,1) Fixed-Window      | 0.9463 | 0.6555 |
| Near-the-money     |       3 | APARCH(1,1) Rolling-Window    | 0.9588 | 0.6521 |
| Moderately OTM/ITM |       1 | GJR-GARCH(1,1) Rolling-Window | 0.6557 | 0.4175 |
| Moderately OTM/ITM |       2 | APARCH(1,1) Rolling-Window    | 0.6967 | 0.4306 |
| Moderately OTM/ITM |       3 | APARCH(1,1) Fixed-Window      | 0.6972 | 0.4348 |
| Deep OTM/ITM       |       1 | GJR-GARCH(1,1) Rolling-Window | 0.5804 | 0.3443 |
| Deep OTM/ITM       |       2 | APARCH(1,1) Rolling-Window    | 0.5834 | 0.3426 |
| Deep OTM/ITM       |       3 | EGARCH(1,1) Fixed-Window      | 0.5856 | 0.3442 |

: Top 3 Modelos por RMSE en cada categoría de moneyness
...
: Comparación de MAE por Categoría

![Comparación de MAE entre GJR-GARCH Rolling, Volatilidad Histórica y ARCH Rolling](figures/fig-mae_comparison.png){#fig-mae-comparison}
...
Por último, la @fig-error-reduction muestra que incorporar volatilidad condicional mediante modelos como el GJR-GARCH aporta mejoras sustanciales en la valuación de opciones frente al uso de volatilidad histórica constante. En todas las categorías analizadas, los errores de valuación disminuyen de manera consistente, alcanzando reducciones de hasta 46.40% en MAE y 49.85% en RMSE. Estos resultados confirman que la volatilidad en los mercados financieros no es estática, sino dinámica y dependiente de la información pasada, por lo que asumir una volatilidad histórica fija limita significativamente la precisión de los modelos tradicionales.

![Reducción porcentual de errores del mejor modelo frente a volatilidad histórica por categoría de moneyness](figures/fig_improvement.png){#fig-error-reduction}

# Conclusión

Los resultados de este proyecto muestran que la elección del “mejor” modelo GARCH depende directamente del objetivo de evaluación. En términos de ajuste *in-sample*, APARCH(1,1) con distribución Skewed Student-t fue el modelo más sólido, ya que capturó con mayor flexibilidad la dinámica histórica de los retornos. En el pronóstico estadístico *out-of-sample*, EGARCH(1,1) con ventana fija obtuvo el mejor desempeño frente a la varianza realizada, lo que confirma que una especificación asimétrica y relativamente parsimoniosa puede ser altamente competitiva cuando el criterio central es minimizar la pérdida de volatilidad. Sin embargo, al trasladar los pronósticos de volatilidad a una aplicación financiera concreta, el resultado más relevante cambia; el modelo GJR-GARCH(1,1) con *rolling window* consiguió un mejor desempeño económico, al obtener el menor error de *pricing* en las cuatro categorías de *moneyness* evaluadas.

Este resultado es importante porque confirma que precisión estadística y valor económico no son equivalentes. Un modelo puede ajustarse mejor a los retornos históricos o pronosticar mejor la varianza realizada y, aun así, no ser el que genera precios de opciones más cercanos al mercado. La principal contribución del proyecto está precisamente en mostrar que evaluar modelos de volatilidad únicamente con criterios estadísticos puede ser insuficiente cuando el objetivo final es una aplicación financiera. En este caso, el criterio económico permitió identificar que el modelo más útil para *pricing* no fue el más complejo ni el de mejor ajuste *in-sample*, sino aquel que tradujo mejor la información de volatilidad hacia precios teóricos.

La superioridad de GJR-GARCH en *pricing* puede explicarse por su capacidad para capturar el efecto asimétrico tipo leverage. En mercados accionarios, los choques negativos suelen tener un impacto mayor sobre la volatilidad futura que los choques positivos de magnitud similar. Esta característica es especialmente relevante para opciones, porque su valor depende no solo del nivel esperado de volatilidad, sino también de cómo el *market* incorpora episodios de riesgo, caídas del subyacente y cambios en la distribución esperada de retornos. Al combinar esta respuesta asimétrica con una estimación rolling, GJR-GARCH logra adaptarse mejor a condiciones recientes de *market* sin depender de una flexibilidad paramétrica excesiva. Por eso APARCH puede dominar en ajuste histórico, pero GJR-GARCH resulta más consistente cuando el objetivo es reducir errores de valuación.

Desde la aplicación financiera, el hallazgo más claro es que incorporar volatilidad condicional dinámica mejora de forma consistente el *pricing* frente al *benchmark* clásico de volatilidad histórica de 252 días. La mejora fue especialmente fuerte en opciones *at-the-money* y *near-the-money*, donde la sensibilidad del precio a cambios en volatilidad es mayor. En cambio, para opciones deep *OTM/ITM*, la ganancia fue más limitada, lo cual es financieramente razonable ya que en estos contratos, una parte importante del precio está determinada por el valor intrínseco o por probabilidades de ejercicio muy bajas, de modo que sofisticar la estimación de volatilidad aporta menos valor marginal.

El alcance del proyecto no debe entenderse como una comparación restringida a Black-Scholes, sino como una prueba de la idea que cuando un modelo financiero depende de un *input* de volatilidad, sustituir una medida histórica no condicional por una estimación condicional dinámica puede mejorar la calidad de la decisión. En este trabajo dicha idea se evaluó mediante *pricing* de opciones, pero el mismo enfoque puede trasladarse a aplicaciones como VaR, *Expected Shortfall*, *stress testing*, cobertura, asignación de portafolios o simulación de escenarios.

Las limitaciones del estudio abren oportunidades claras para investigación futura. Primero, sería útil replicar la metodología en otros índices, acciones individuales o mercados internacionales para evaluar si la superioridad de GJR-GARCH en *pricing* se mantiene bajo distintas estructuras de liquidez, volatilidad y composición sectorial. Segundo, futuras extensiones podrían incorporar una comparación más detallada por vencimiento, tipo de opción y régimen de *market*, dado que la utilidad de cada modelo puede variar entre periodos de calma y episodios de estrés financiero. Tercero, sería relevante complementar la evaluación de *pricing* con métricas de cobertura, ya que un modelo que minimiza errores de precio no necesariamente minimiza errores de *hedge*. Finalmente, una extensión natural sería comparar este enfoque *plug-in* con modelos de valuación que incorporen la dinámica GARCH directamente bajo una medida de *pricing*.

En conjunto, los resultados apoyan el uso de modelos GARCH asimétricos para aplicaciones financieras de valuación de opciones. La evidencia muestra que modelar explícitamente la persistencia, la reversión y la respuesta asimétrica de la volatilidad aporta información económicamente útil frente a una volatilidad histórica simple. Bajo el diseño empírico de este proyecto, GJR-GARCH con *rolling window* ofrece el mejor balance entre adaptación a condiciones recientes, captura del efecto leverage y precisión en *pricing*.
...
# Apéndices {.appendix}
## Apéndice A: Errores en Pronósticos de Varianza Realizada {#sec-apendice-a}
![Error Varianza Realizada vs Varianza Pronosticada ARCH(1) Fixed-Window](figures/fig-error-ARCH(1) (Fixed-Window).png){#fig-error-arch-fixed}
![Error Varianza Realizada vs Varianza Pronosticada ARCH(1) Rolling-Window](figures/fig-error-ARCH(1) (Rolling-Window).png){#fig-error-arch-rolling}
![Error Varianza Realizada vs Varianza Pronosticada GARCH(1,1) Fixed-Window](figures/fig-error-GARCH(1,1) (Fixed-Window).png){#fig-error-garch-fixed}
![Error Varianza Realizada vs Varianza Pronosticada GARCH(1,1) Rolling-Window](figures/fig-error-GARCH(1,1) (Rolling-Window).png){#fig-error-garch-rolling}
![Error Varianza Realizada vs Varianza Pronosticada EGARCH(1,1) Fixed-Window](figures/fig-error-EGARCH(1,1) (Fixed-Window).png){#fig-error-egarch-fixed}
![Error Varianza Realizada vs Varianza Pronosticada EGARCH(1,1) Rolling-Window](figures/fig-error-EGARCH(1,1) (Rolling-Window).png){#fig-error-egarch-rolling}
![Error Varianza Realizada vs Varianza Pronosticada GJR-GARCH(1,1) Fixed-Window](figures/fig-error-GJR-GARCH(1,1) (Fixed-Window).png){#fig-error-gjr-fixed}
![Error Varianza Realizada vs Varianza Pronosticada GJR-GARCH(1,1) Rolling-Window](figures/fig-error-GJR-GARCH(1,1) (Rolling-Window).png){#fig-error-gjr-rolling}
![Error Varianza Realizada vs Varianza Pronosticada APARCH(1,1) Fixed-Window](figures/fig-error-APARCH(1,1) (Fixed-Window).png){#fig-error-aparch-fixed}
![Error Varianza Realizada vs Varianza Pronosticada APARCH(1,1) Rolling-Window](figures/fig-error-APARCH(1,1) (Rolling-Window).png){#fig-error-aparch-rolling}

```{=latex}
\clearpage
```

## Apéndice B: Dispersión Relativa de Varianza Pronosticada frente Varianza Realizada  {#sec-apendice-b}

```{=latex}
\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth,height=0.42\textheight,keepaspectratio]{figures/fig-loglog-ARCH(1) (Fixed-Window).png}
\vspace{0.6em}
\includegraphics[width=0.95\linewidth,height=0.42\textheight,keepaspectratio]{figures/fig-loglog-ARCH(1) (Rolling-Window).png}
\end{figure}
```
{{< pagebreak >}}
```{=latex}
\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth,height=0.42\textheight,keepaspectratio]{figures/fig-loglog-GARCH(1,1) (Fixed-Window).png}
\vspace{0.6em}
\includegraphics[width=0.95\linewidth,height=0.42\textheight,keepaspectratio]{figures/fig-loglog-GARCH(1,1) (Rolling-Window).png}
\end{figure}
```
{{< pagebreak >}}
```{=latex}
\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth,height=0.42\textheight,keepaspectratio]{figures/fig-loglog-EGARCH(1,1) (Fixed-Window).png}
\vspace{0.6em}
\includegraphics[width=0.95\linewidth,height=0.42\textheight,keepaspectratio]{figures/fig-loglog-EGARCH(1,1) (Rolling-Window).png}
\end{figure}
```
{{< pagebreak >}}
```{=latex}
\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth,height=0.42\textheight,keepaspectratio]{figures/fig-loglog-GJR-GARCH(1,1) (Fixed-Window).png}
\vspace{0.6em}
\includegraphics[width=0.95\linewidth,height=0.42\textheight,keepaspectratio]{figures/fig-loglog-GJR-GARCH(1,1) (Rolling-Window).png}
\end{figure}
```
{{< pagebreak >}}
```{=latex}
\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth,height=0.42\textheight,keepaspectratio]{figures/fig-loglog-APARCH(1,1) (Fixed-Window).png}
\vspace{0.6em}
\includegraphics[width=0.95\linewidth,height=0.42\textheight,keepaspectratio]{figures/fig-loglog-APARCH(1,1) (Rolling-Window).png}
\end{figure}
```
